...
%
Cet opérateur va essayer de créer des groupes de 8 tâches assez proche dans le graphe.
%
Nous allons donc diviser par 8 le nombre de tâches, ce qui peut paraître peu mais cette valeur a été choisi empiriquement parmi un ensemble de valeur.
%
...
%
Par contre, avec 8 variables primaires, on obtient un gain de performance qui est dû à une meilleur utilisation des caches, surtout du cache L2 avec une diminution de 10\% des défauts de cache.
...
%
En effet, avec l'opérateur F, le nombre de tâches était fortement réduit mais les tâches agrégés n'avait pas une bonne réutilisabilité des données en cache.
%
Au contraire, l'opérateur D ne réduisait que de très peu le donc de tâches mais améliorait la réutilisation des caches.
%
L'opérateur C offre le meilleur des deux opérateurs, il produit peu de tâches et en plus ces tâches réutilise correctement les données en cache.
...
%
Nous expliquerons ce soucis de performance dans les parties suivantes de la thèse.
